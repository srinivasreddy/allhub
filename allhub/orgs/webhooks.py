...
class WebHooksMixin:
    ...
    def hook(self, org, hook_id):
        url = f"/orgs/{org}/hooks/{hook_id}"
        self.response = Response(self.get(url), "WebHook")
        return self.response.transform()

    def create_hook(self, org, name, config, events=("push",), active=True):
    ...
    def edit_hook(self, org, hook_id, config, events=("push",), active=True):
        assert isinstance(config, dict)
        url = f"/orgs/{org}/hooks/{hook_id}"
    ...
    def ping_hook(self, org, hook_id):
